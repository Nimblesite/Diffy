...
const readDiffly = async (uriString: string): Promise<string> => {
  const doc = await vscode.workspace.openTextDocument(vscode.Uri.parse(uriString));
  return doc.getText();
};

describe("DifflyContentProvider (real diffly:// URIs against the seeded repo)", () => {
  before(async () => {
  ...
  it("resolves dir/c.txt at v1 and v2 with the expected text", async () => {
    const shas = readSeedShas();
    const c1 = await readDiffly(`diffly://commit/${shas.first}/dir/c.txt`);
    assert.match(c1, /c\.txt v1/);
    const c2 = await readDiffly(`diffly://commit/${shas.second}/dir/c.txt`);
  ...
  it("returns the renamed file b2.txt at the third commit but not before", async () => {
    const shas = readSeedShas();
    const b2 = await readDiffly(`diffly://commit/${shas.third}/b2.txt`);
    assert.match(b2, /renamed and extended/);
    const missing = await readDiffly(`diffly://commit/${shas.first}/b2.txt`);
  ...
  it("returns empty string for a deleted path (d.txt at commit 2)", async () => {
    const shas = readSeedShas();
    const deletedAtV2 = await readDiffly(`diffly://commit/${shas.second}/d.txt`);
    assert.equal(deletedAtV2, "");
    const aliveAtV1 = await readDiffly(`diffly://commit/${shas.first}/d.txt`);
    assert.match(aliveAtV1, /d\.txt v1/);
  });

  it("returns empty string for a malformed diffly URI", async () => {
    const bad = await readDiffly("diffly://nonsense/whatever");
    assert.equal(bad, "");
  });
});
